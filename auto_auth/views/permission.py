...
from auto_auth.forms import PermissionListForm
...
def SupperVerify():
    ...
    def decorator(view_func):
        def _wrapped_view(request, *args, **kwargs):
            uuser = User.objects.get(username=request.user)
            auser = AuthUser.objects.get(user__username =request.user)
            if not uuser.is_superuser: #判断用户如果是超级管理员则具有所有权限
...
def PermissionVerify():
    ...
    def decorator(view_func):
        def _wrapped_view(request, *args, **kwargs):
            uuser = User.objects.get(username=request.user)
            auser = AuthUser.objects.get(user__username =request.user.username)
            if not uuser.is_superuser: #判断用户如果是超级管理员则具有所有权限
                if not auser.role: #如果用户无角色，直接返回无权限
                    return HttpResponseRedirect(reverse('permissiondenyurl'))

                role_permission = RoleGroup.objects.get(name=auser.role)
...
def NoPermission(request):
    ...
    return render_to_response('permissiondeny.html',kwvars,RequestContext(request))
...
def AddPermission(request):
    ...
    return render_to_response('auto_auth/permission.add.html',kwvars,RequestContext(request))
...
#列出组权限
    ...
    return render_to_response('auto_auth/permission.list.html',kwvars,RequestContext(request))
...
#编辑组权限
    ...
    return render_to_response('auto_auth/permission.edit.html',kwvars,RequestContext(request))
